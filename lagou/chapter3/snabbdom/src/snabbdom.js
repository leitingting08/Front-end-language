...
function changeSort (e, prop) {
    sortBy = prop.data.on.click[1]
    data.sort((a, b) => {
  ...
  function remove (e, movie) {
    data = data.filter((m) => {
      return m !== movie.data.on.click[1]
    })
    render()
  }
  ...
  function movieView (movie) {
    return h('div.row', {
      ...
      style: {
        ...
        delayed: { transform: `translateY(${movie.offset}px)`, display: 'flex' },
      ...
      h('div', { style: { fontWeight: 'bold', padding: '0 10px' } }, movie.rank),
      h('div', { style: { padding: '0 10px' } }, movie.title),
      h('div', movie.desc),
      h('div.btn.rm-btn', { style: { margin: '0 20px', cursor: 'pointer' }, on: { click: [remove, movie] } }, 'x'),
    ])
  }
  ...
  function view (data) {
    return h('div', [
      ...
      h('div', [
        'Sort by: ',
        h('span.btn-group', [
          h('a.btn.rank', { style: { marginRight: '20px', cursor: 'pointer' }, class: { active: sortBy === 'rank' }, on: { click: [changeSort, 'rank'] } }, 'Rank'),
          h('a.btn.title', { style: { marginRight: '20px', cursor: 'pointer' }, class: { active: sortBy === 'title' }, on: { click: [changeSort, 'title'] } }, 'Title'),
          h('a.btn.desc', { style: { marginRight: '20px', cursor: 'pointer' }, class: { active: sortBy === 'desc' }, on: { click: [changeSort, 'desc'] } }, 'Description'),
        ]),
        h('a.btn.add',{ style: { float: 'right', cursor: 'pointer' }, on: { click: add } }, 'Add'),
      ]),
